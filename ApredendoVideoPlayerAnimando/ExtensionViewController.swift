    ...
    func begin(gestureRecognizer: UIPanGestureRecognizer) {
        if let playerVideo = self.playerVideo {
            ...
            playerVideo.viewBG!.frame.origin.x =
                playerVideo.frame.origin.x == 0 ? 0 : ( self.view.bounds.width / 2 )
            
            playerVideo.frame.size.width = self.view.bounds.width
            playerVideo.frame.origin.x = 0
            
            playerVideo.frame.size.height = self.view.bounds.height
        ...
        if let playerVideo = self.playerVideo {
            if (gestureRecognizer.translation(in: playerVideo).y >= ( self.view.bounds.height / 3 )) {
                ...
                UIView.animate(withDuration: 0.5, animations: {
                    ...
                    playerVideo.frame.origin.x = self.view.bounds.width / 2
